    ...
    <View style={{ backgroundColor: "#e2e9f1", flex: 1 }}>
      <View style={{ width: "100%", backgroundColor: "#227ffd", flexDirection: "row", padding: 10, marginTop: 25, justifyContent: "space-between" }}>
        <View style={{ flexDirection: "row", alignItems: "center" }}>
          ...
          <Text style={{ color: "white", margin: 10, fontSize: 17, fontWeight: 600 }}>{userChat.username}</Text>
        </View>
            ...
            if (item.senderid._id != user._id && item.senderid != user._id) {
              if (item.status == 1) {
                return <View style={{ flexDirection: "row", marginBottom: 10 }} key={uuidv4()}>
                  <Image source={{ uri: userChat.avatarpicture }} style={{ height: 40, width: 40, borderRadius: 20 }} />
                ...
                return <View style={{ flexDirection: "column", marginBottom: 10, marginLeft: 10 ,alignSelf:"flex-start"}} key={uuidv4()}>
                  <View
                    ...
                    >
                      {isImageUrl(item.content) ? (
                        <Image source={{ uri: item.content }} style={{ width: 200, height: 200 }} />
                            ...
                            <Text style={{ fontSize: 17, paddingHorizontal: 7, paddingVertical: 10, backgroundColor: "white", borderRadius: 10, minWidth: 90 }} key={item._id}>
              ...
              if (item.status == 0)
                return <TouchableOpacity style={{ flexDirection: "row", marginBottom: 10,alignSelf:"flex-end" }} key={uuidv4()}
                  onLongPress={() => {
                    setModalVisible(true)
                    setItemModal(item)
                  }}
      ...
      <Modal
        ...
        <View style={{ backgroundColor: 'rgba(0, 0, 0, 0.5)', flex: 1, justifyContent: "flex-end", padding: 15 }}>
          ...
          <View style={{ backgroundColor: "white", height: 100, borderRadius: 15, flexDirection: "row", alignItems: "center", justifyContent: "space-around" }}>
            ...
            {!itemModal.senderid?"":itemModal.senderid._id==user._id?
            <TouchableOpacity style={{ alignItems: "center" }}
